...
# 运行单个测试函数
uv run pytest tests/test_analyze.py::test_czsc_basic -v
...
1. **`czsc._native`** - PyO3 编译产生的 Rust 扩展模块（缠论核心）：
   ...
   - 暴露 30+ 信号函数（按 `czsc._native.signals.{bar,cxt,tas,vol,pressure,obv,cvolp}` 分组）
...
2. **`crates/`** - Rust workspace（9 个 crate，详见 `docs/MIGRATION_NOTES.md` §1）：
   - `czsc-core` / `czsc-signals` / `czsc-trader` / `czsc-utils` / `czsc-ta`
   - `czsc-signal-macros` / `error-macros` / `error-support`（proc-macro / 错误支持）
...
4. **`czsc/_native.signals`** - 信号函数（Rust 实现，通过 PyO3 暴露）：
   - 按类别分组：`{bar,cxt,tas,vol,pressure,obv,cvolp}` 共 30+ 信号函数
...
- **类型 stub**：`czsc/py.typed` 启用 inline 类型注解；`czsc/_native.pyi` 由 `pyo3-stub-gen` 自动生成（计划中，详见 `docs/MIGRATION_NOTES.md` §10.2）
- **rs-czsc 关系**：czsc 一次性 fork rs-czsc 的 Rust 实现进本仓库，**不再做季度同步**；基线 commit 与 czsc-only 改动记录在 `docs/MIGRATION_NOTES.md`
