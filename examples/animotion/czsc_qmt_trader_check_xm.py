...
pool_path = 'C:\\czsc_realtime\\pools'
...
def get_pools():
    """
    用sdt,edt，得到所有股票池在此期间的结果 和 期间最后一组选股结果
    """
    pools = []

    pathnames = glob.glob(os.path.join(pool_path, '*.csv'))
    pools = [os.path.basename(pathname)[:-4] for pathname in pathnames]
    return pools


@st.cache_data
def get_pool_symbols(poolname, sdt_, edt_):
    """
    用sdt,edt，得到所有股票池在此期间的结果 和 期间最后一组选股结果
    """
