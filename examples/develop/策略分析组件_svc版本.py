...
def show_cta_periods_classify_demo():
    ...
    with col5:
        weight_type = st.selectbox("权重类型", ['ts', 'cs'], index=0, 
                                 help="ts: 时序权重, cs: 截面权重")
...
def show_cta_periods_classify_advanced_demo():
    ...
    with tab1:
        ...
        selected_param = st.selectbox("选择参数组合", 
                                    [p["name"] for p in param_sets])
        ...
        show_cta_periods_classify(
            ...
            digits=2,
            weight_type='ts',
            q1=current_params['q1'],
            q2=current_params['q2']
        )
    ...
    with tab2:
        ...
        weight_type = st.radio("选择权重类型", ['ts', 'cs'], 
                             help="ts: 时序权重，cs: 截面权重")
...
def show_volatility_classify_demo():
    ...
    col1, col2, col3, col4 = st.columns(4)
    with col1:
        kind = st.selectbox("分类方式", ['ts', 'cs'], index=0, 
                          help="ts:时序分类, cs:截面分类")
    with col2:
        fee_rate = st.slider("手续费率", 0.0001, 0.001, 0.0002, 0.0001, format="%.4f")
    with col3:
    ...
    with col6:
        weight_type = st.selectbox("权重类型", ['ts', 'cs'], index=0, 
                                 help="ts: 时序权重, cs: 截面权重", key="vol_weight")
...
def show_volatility_classify_advanced_demo():
    ...
    with tab1:
        ...
        st.info(f"当前窗口期: {selected_window}天 - " + 
               ("短期波动" if selected_window <= 15 else 
                "中期波动" if selected_window <= 30 else "长期波动"))
    ...
    with tab2:
        ...
        selected_q = st.selectbox("选择分位数设置", 
                                [q["name"] for q in quantile_sets], key="q_select")
    ...
    with tab3:
        ...
        classify_kind = st.radio("选择分类方式", ['ts', 'cs'], 
...
def main():
    ...
    # 分组展示演示类型
    demo_category = st.sidebar.selectbox(
        "选择分析类别",
        ["基础策略分析", "市场环境分析", "高级案例分析"]
    )
    
    if demo_category == "基础策略分析":
        demo_options = ["策略收益贡献分析", "最近N天策略表现", "季节性收益对比", 
                       "组合绩效分析", "换手率分析", "策略绩效对比", "品种基准分析"]
