...
if not os.path.exists(file_token):
    print(
        "{} 文件不存在，请单独启动一个 python 终端，调用 set_gm_token 方法创建该文件，再重新执行。".format(file_token)
    )
else:
...
def get_kline(symbol, end_time, freq="60s", count=33000, adjust=ADJUST_PREV):
    ...
    freq_map_ = {"60s": Freq.F1, "300s": Freq.F5, "900s": Freq.F15, "1800s": Freq.F30, "3600s": Freq.F60, "1d": Freq.D}
    ...
    if exchange in ["SZSE", "SHSE"]:
        ...
        df = history_n(
            ...
            end_time=end_time,
            adjust=adjust,
            fields="symbol,eob,open,close,high,low,volume,amount,position",
            count=count,
            df=True,
        )
...
def get_init_bg(
    symbol: str, end_dt: [str, datetime], base_freq: str, freqs: List[str], max_count=1000, adjust=ADJUST_PREV
):
    ...
    bars2 = get_kline(symbol, end_dt, freq_cn2gm[base_freq], count=int(240 / int(base_freq.strip("分钟")) * delta_days))
...
def report_account_status(context):
    ...
    cash_report = (
        f"净值：{int(cash.nav)}，可用资金：{int(cash.available)}，"
    ...
    for p in positions:
        p_report = (
            f"标的：{p.symbol}，名称：{context.stocks.get(p.symbol, '无名')}，"
    ...
    if context.mode != MODE_BACKTEST:
        ...
        for symbol, info in context.symbols_info.items():
            ...
            if "日线" in trader.kas.keys():
                ...
                row.update({"昨日收盘": round(bar1.close, 2), "今日涨幅": round(bar2.close / bar1.close - 1, 4)})

            if p:
                row.update(
                    {"实盘持仓数量": p.volume, "实盘持仓成本": round(p.vwap, 2), "实盘持仓市值": int(p.volume * p.vwap)}
                )
            else:
...
def init_context_universal(context, name):
    ...
    logger.add(
        os.path.join(data_path, "gm_trader.log"), rotation="500MB", encoding="utf-8", enqueue=True, retention="1 days"
    )
    logger.info("运行配置：")
...
def init_context_traders(context, symbols: List[str], strategy):
    ...
    for symbol in symbols:
        try:
            ...
            logger.info(
                "{} Trader 构建成功，最新时间：{}，多仓：{}".format(
                    symbol, trader.end_dt, trader.get_ensemble_pos("mean")
                )
            )
            ...
            logger.exception(
                f"{e1}：{symbol} - {context.stocks.get(symbol, '无名')} 初始化失败，当前时间：{context.now}"
            )
