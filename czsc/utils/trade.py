...
def stoploss_by_direction(dfw: pd.DataFrame, stoploss: float = 0.08) -> pd.DataFrame:
    """按方向止损改写权重序列。

    把权重序列按"方向连续段"切成若干笔交易（``order_id``），对每一笔逐 bar
    计算从入场价开始的方向化累计收益 ``hold_returns``；当累计回撤达到
    ``-stoploss`` 时，从该 bar 起把后续 ``weight`` 强制归零、并把 ``is_stop``
    置 ``True``，等价于"按方向止损平仓"。原始权重保留在 ``raw_weight`` 列。

    历史上该函数由 ``rs_czsc.stoploss_by_direction`` 提供；rs_czsc 后续未再
    维护，czsc 在迁移过程中以 czsc-only 改动新增此 Python 实现，作为
    ``czsc.svc.backtest.show_stoploss_by_direction`` 的底层依赖。

    :param dfw: 包含 ``dt`` / ``symbol`` / ``weight`` / ``price`` 列的权重数据
    :param stoploss: 触发止损的浮亏阈值（取正值；例如 0.08 代表 -8%）
    :return: 复制自 ``dfw``，额外带 ``raw_weight`` / ``order_id`` /
        ``hold_returns`` / ``min_hold_returns`` / ``returns`` / ``is_stop`` 列
    """
    out = dfw.copy()
    out = out.sort_values(["symbol", "dt"]).reset_index(drop=True)
    out["raw_weight"] = out["weight"].astype(float)

    sign = out["weight"].apply(lambda w: 0 if w == 0 else (1 if w > 0 else -1))
    # order_id 在方向变化时自增；在每个 symbol 内独立编号
    direction_change = sign.ne(sign.groupby(out["symbol"]).shift(1)).astype(int)
    out["order_id"] = direction_change.groupby(out["symbol"]).cumsum()

    out["hold_returns"] = 0.0
    out["min_hold_returns"] = 0.0
    out["returns"] = 0.0
    out["is_stop"] = False

    for (_symbol, _oid), grp in out.groupby(["symbol", "order_id"], sort=False):
        weight_sign = sign.loc[grp.index].iloc[0]
        prices = grp["price"].astype(float).to_numpy()
        idx = grp.index

        if weight_sign == 0 or len(prices) == 0:
            continue

        entry_price = prices[0]
        # 方向化累计收益：long = price/entry - 1; short = 1 - price/entry
        hold = (prices / entry_price - 1.0) * weight_sign
        # 单 bar 收益（方向化）：长仓 = price[t]/price[t-1] - 1, 短仓取负
        bar_ret = pd.Series(prices, index=idx).pct_change().fillna(0.0).to_numpy() * weight_sign
        running_min = pd.Series(hold).cummin().to_numpy()

        out.loc[idx, "hold_returns"] = hold
        out.loc[idx, "min_hold_returns"] = running_min
        out.loc[idx, "returns"] = bar_ret

        # 止损触发条件：min_hold_returns ≤ -stoploss
        # 加一个极小容差，避免浮点除法（如 92/100 - 1 = -0.07999999999999996）让
        # 边界值落在止损线另一侧。
        triggered = running_min <= -float(stoploss) + 1e-9
        if triggered.any():
            first_stop = int(triggered.argmax())
            stop_idx = idx[first_stop:]
            out.loc[stop_idx, "is_stop"] = True
            out.loc[stop_idx, "weight"] = 0.0
