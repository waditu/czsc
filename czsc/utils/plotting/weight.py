...
def plot_weight_histogram_kde(
    ...
    for i, (position_type, (row, col)) in enumerate(zip(positions, position_indices, strict=False)):
...
def plot_weight_cdf(
    ...
    fig_cdf.update_layout(
        ...
        width=width,
        showlegend=True,
        legend={"x": 0.02, "y": 0.98, "bgcolor": "rgba(255,255,255,0.8)", "bordercolor": "black", "borderwidth": 1},
        plot_bgcolor="white",
        paper_bgcolor="white",
    )
    ...
    fig_cdf.update_xaxes(
        linecolor="black", linewidth=2, tickfont={"size": 12}, showgrid=True, gridwidth=1, gridcolor="lightgray"
    )
...
def plot_turnover_overview(dfw: pd.DataFrame, title: str = "策略换手率分析总览", height: int = 600) -> go.Figure:
    ...
    # 更新坐标轴样式
    fig.update_xaxes(
        showgrid=True, gridwidth=1, gridcolor="lightgray", linecolor="black", linewidth=2, tickfont={"size": 12}
    )
