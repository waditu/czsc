...
def _create_pdf_drawdown(dret, ret_col="total", benchmark_returns=None):
    ...
    fig.add_trace(
        go.Scatter(
            x=drawdown.index,
            y=drawdown * 100,
            fillcolor="rgba(250,128,114,0.4)",
            line={"color": "salmon"},
            fill="tozeroy",
            mode="lines",
            name="回撤 (%)",
...
def _create_pdf_daily_distribution(dret, ret_col="total"):
    ...
    for sigma in [-3, -2, -1, 1, 2, 3]:
        ...
        fig.add_vline(
            ...
            annotation_font={"color": "red", "size": 9},
...
def _create_long_short_table_figure(stats_df):
    ...
    fig.update_layout(height=250, margin={"l": 40, "r": 40, "b": 20, "t": 50})
    return fig
...
def _build_and_save_pdf_report(
    title: str, df: pd.DataFrame, config: dict, metrics: list, figures: dict, output_path: str, wb=None
) -> None:
    """构建并保存 PDF 报告
