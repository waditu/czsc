...
from czsc.utils.sig import create_single_signal, get_sub_elements, is_symmetry_zs
...
def byi_second_bs_V230324(c: CZSC, **kwargs) -> OrderedDict:
    ...
    **信号列表：**
    ...
    if (
        b9.direction == Direction.Down
        and max(b1_dif, b3_dif, b5_dif) < 0
    ...
    if (
        b9.direction == Direction.Up
        and min(b1_dif, b3_dif, b5_dif) > 0
