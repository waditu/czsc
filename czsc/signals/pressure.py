"""czsc.signals.pressure —— 价格压力（``pressure_*``）信号命名空间。

本模块对外暴露所有以 ``pressure_`` 为前缀的信号函数，主要用于刻画市场中的
压力位/支撑位信息，例如近期高低点的密集成交区、价位的反复测试次数、突破
后的回踩验证等。这些信号常配合 ``cxt_*`` 与 ``vol_*`` 一起用于做出场和加仓
判断。

实现位置
========
真正的计算逻辑由 Rust 实现，源文件位于 ``crates/czsc-signals/src/pressure.rs``，
通过 ``#[signal(...)]`` 宏在编译期登记到 ``inventory`` 全局表里。Python 端
仅做轻量级转发，避免 Python 解释器层面的开销。

调用约定
========
- 所有 ``pressure_*_VyymmDD`` 函数都是“按需暴露”的：访问
  ``czsc.signals.pressure.pressure_xxx_V230101`` 时由 :func:`__getattr__`
  临时合成 Python 闭包；
...
# 当前子模块对应的类别前缀；所有以 ``pressure_`` 开头的信号会归到这里
_CATEGORY = "pressure"


def list_signals() -> list[str]:
    """列出 Rust 清单中所有 ``pressure_*`` 信号的名称。

    Returns
    -------
    list[str]
        以 ``pressure_`` 为前缀的信号名称列表，按字典序排序。
    """
    return _list_signals(_CATEGORY)


def get_signal_template(name: str) -> str | None:
    """获取某个 ``pressure_*`` 信号的参数模板字符串。
...
def __getattr__(name: str):
    """按需暴露每个已注册的 ``pressure_*`` 信号为可调用对象。

    访问 ``czsc.signals.pressure.pressure_xxx_V230101`` 时本函数被自动触发；
    ...
    ------
    AttributeError
        当 ``name`` 不属于 ``pressure_*`` 信号或未在 Rust 清单中注册时抛出。
    """
    if name.startswith(_CATEGORY + "_") and name in _list_signals(_CATEGORY):
        return _make_signal_callable(name)
    raise AttributeError(f"module 'czsc.signals.pressure' has no attribute {name!r}")
