"""czsc.signals.obv —— 能量潮（``obv_*``）信号命名空间。

本模块对外暴露所有以 ``obv_`` 为前缀的信号函数。OBV（On-Balance Volume，能量潮）
是一类把成交量按收盘价方向逐笔累加的指标，用以衡量“量在价先”的资金推动强度，
常见衍生信号包括 OBV 与价格的背离、OBV 均线穿越、OBV 趋势斜率等。

实现位置
========
真正的计算逻辑由 Rust 实现，源文件位于 ``crates/czsc-signals/src/obv.rs``，
通过 ``#[signal(...)]`` 宏在编译期登记到 ``inventory`` 全局表里。Python 端
仅做轻量级转发，避免 Python 解释器层面的开销。

调用约定
========
- 所有 ``obv_*_VyymmDD`` 函数都是“按需暴露”的：访问 ``czsc.signals.obv.obv_xxx_V230101``
...
# 当前子模块对应的类别前缀；所有以 ``obv_`` 开头的信号会归到这里
_CATEGORY = "obv"


def list_signals() -> list[str]:
    """列出 Rust 清单中所有 ``obv_*`` 信号的名称。

    Returns
    -------
    list[str]
        以 ``obv_`` 为前缀的信号名称列表，按字典序排序。
    """
    return _list_signals(_CATEGORY)


def get_signal_template(name: str) -> str | None:
    """获取某个 ``obv_*`` 信号的参数模板字符串。
...
def __getattr__(name: str):
    """按需暴露每个已注册的 ``obv_*`` 信号为可调用对象。

    访问 ``czsc.signals.obv.obv_xxx_V230101`` 时本函数被自动触发；
    ...
    ------
    AttributeError
        当 ``name`` 不属于 ``obv_*`` 信号或未在 Rust 清单中注册时抛出。
    """
    if name.startswith(_CATEGORY + "_") and name in _list_signals(_CATEGORY):
        return _make_signal_callable(name)
    raise AttributeError(f"module 'czsc.signals.obv' has no attribute {name!r}")
