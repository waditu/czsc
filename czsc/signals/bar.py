"""czsc.signals.bar —— K 线级别（``bar_*``）信号命名空间。

本模块对外暴露所有以 ``bar_`` 为前缀的信号函数，覆盖 K 线层面的常用统计量，
例如成交额累计、振幅、跳空、上下影线、连续阴/阳线等基础形态。这些信号是
事件（Events）和持仓（Positions）等高层逻辑组合的基础积木。

实现位置
========
真正的计算逻辑由 Rust 实现，源文件位于 ``crates/czsc-signals/src/bar.rs``，
通过 ``#[signal(...)]`` 宏在编译期登记到 ``inventory`` 全局表里。Python 端
仅做轻量级转发，避免 Python 解释器层面的开销。

调用约定
========
- 所有 ``bar_*_VyymmDD`` 函数都是“按需暴露”的：访问 ``czsc.signals.bar.bar_xxx_V230101``
  时由 :func:`__getattr__` 临时合成 Python 闭包；
- 闭包签名为 ``fn(czsc, params)``，其中 ``params`` 是参数字典；
- 返回值为 ``list[Signal]``；
- 使用 :func:`list_signals` 可枚举所有可用名称，便于做配置校验或自动注册。
"""
...
# 当前子模块对应的类别前缀；所有以 ``bar_`` 开头的信号会归到这里
_CATEGORY = "bar"


def list_signals() -> list[str]:
    """列出 Rust 清单中所有 ``bar_*`` 信号的名称。

    Returns
    -------
    list[str]
        以 ``bar_`` 为前缀的信号名称列表，按字典序排序。
    """
    return _list_signals(_CATEGORY)


def get_signal_template(name: str) -> str | None:
    """获取某个 ``bar_*`` 信号的参数模板字符串。

    Parameters
    ----------
    name : str
        完整的信号名称，例如 ``"bar_amount_acc_V230214"``。
...
def __getattr__(name: str):
    """按需暴露每个已注册的 ``bar_*`` 信号为可调用对象。

    访问 ``czsc.signals.bar.bar_xxx_V230101`` 时本函数被自动触发；
    若名称在 Rust 清单中存在，则合成一个 Python 闭包返回；否则抛出
    :class:`AttributeError`，与普通模块属性查找语义保持一致，确保静态
    检查工具（如 mypy、pyright）行为符合预期。
    ...
    ------
    AttributeError
        当 ``name`` 不属于 ``bar_*`` 信号或未在 Rust 清单中注册时抛出。
    """
    if name.startswith(_CATEGORY + "_") and name in _list_signals(_CATEGORY):
        return _make_signal_callable(name)
    raise AttributeError(f"module 'czsc.signals.bar' has no attribute {name!r}")


def __dir__() -> list[str]:
    """自定义 ``dir()`` 输出，让 IDE / REPL 能看到所有动态暴露的信号。

    包含模块级公共函数加上当前 Rust 清单中可用的全部 ``bar_*`` 信号名称。
    """
    return [
        "list_signals",
        "get_signal_template",
