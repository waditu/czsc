"""czsc.signals.tas —— 技术指标（``tas_*``）信号命名空间。

本模块对外暴露所有以 ``tas_`` 为前缀的信号函数。``tas`` 是 “Technical Analysis
Signals” 的缩写，覆盖了基于 ``ta-lib`` 风格指标族的信号，例如 MACD（金叉、
死叉、背驰）、KDJ、RSI、BOLL、MA 多空排列、ATR 波动率等。本子模块是构建
中短期趋势/震荡策略最常用的信号集合之一。

实现位置
========
真正的计算逻辑由 Rust 实现，源文件位于 ``crates/czsc-signals/src/tas.rs``，
通过 ``#[signal(...)]`` 宏在编译期登记到 ``inventory`` 全局表里。Rust 端会
利用 ``TaCache`` 对常见指标做缓存，避免在同一根 K 线上重复计算。Python 端
仅做轻量级转发。

调用约定
========
- 所有 ``tas_*_VyymmDD`` 函数都是“按需暴露”的：访问 ``czsc.signals.tas.tas_xxx_V230101``
...
# 当前子模块对应的类别前缀；所有以 ``tas_`` 开头的信号会归到这里
_CATEGORY = "tas"


def list_signals() -> list[str]:
    """列出 Rust 清单中所有 ``tas_*`` 信号的名称。

    Returns
    -------
    list[str]
        以 ``tas_`` 为前缀的信号名称列表，按字典序排序。
    """
    return _list_signals(_CATEGORY)


def get_signal_template(name: str) -> str | None:
    """获取某个 ``tas_*`` 信号的参数模板字符串。
...
def __getattr__(name: str):
    """按需暴露每个已注册的 ``tas_*`` 信号为可调用对象。

    访问 ``czsc.signals.tas.tas_xxx_V230101`` 时本函数被自动触发；
    ...
    ------
    AttributeError
        当 ``name`` 不属于 ``tas_*`` 信号或未在 Rust 清单中注册时抛出。
    """
    if name.startswith(_CATEGORY + "_") and name in _list_signals(_CATEGORY):
        return _make_signal_callable(name)
    raise AttributeError(f"module 'czsc.signals.tas' has no attribute {name!r}")
