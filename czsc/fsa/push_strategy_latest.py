...
class StrategyCard:
    ...
    def __init__(
        ...
        strategy_name: str,
        dfw: pd.DataFrame,
        out_sample_sdt: str = "20250101",
        describe: str = "",
        show_recent_detail: bool = True,
        **kwargs,
    ):
        """
        ...
        self.wb = WeightBacktest(
            self.dfw.copy(), fee_rate=fee_rate, digits=digits, weight_type=weight_type, yearly_days=yearly_days
        )
    ...
    def _get_out_sample_curve(self) -> list[dict[str, Any]]:
        ...
        df = (
            df_total.merge(df_long, on="date", how="left")
        ...
        for date, row in out_sample_data.iterrows():
            ...
            for col in cols:
                if col in row:
                    chart_data.append({"time": date_str, "type": col, "value": round(row[col] * 100, 2)})
        return chart_data
    ...
    def _build_header(self) -> dict[str, Any]:
        """构建卡片头部"""
        return {
            "title": {"tag": "plain_text", "content": f"{self.strategy_name} - 策略最新表现"},
            "subtitle": {"tag": "plain_text", "content": f"样本外开始时间: {self.out_sample_sdt}"},
            "template": "blue",
        }
                ...
                "columns": [
                    {
                        ...
                        "width": "weighted",
                        "weight": 1,
                        "elements": [{"tag": "markdown", "content": self._format_returns_table(df)}],
                    }
                ],
            }
        ]
    ...
    def _build_positions_section(self, df: pd.DataFrame) -> list[dict[str, Any]]:
        """构建最新持仓表格部分"""
        if df.empty:
            return [{"tag": "markdown", "content": "当前无持仓"}]
                ...
                "columns": [
                    {
                        ...
                        "width": "weighted",
                        "weight": 1,
                        "elements": [{"tag": "markdown", "content": self._format_positions_table(df)}],
                    }
                ],
            }
        ]
    ...
    def _build_stats_section(self, stats_df: pd.DataFrame) -> dict[str, Any]:
        ...
        for i in range(5):
            ...
            for _, row in subset.iterrows():
                elements.append(
                    {"tag": "markdown", "content": f"**{row['指标']}**\n<font color='grey'>{row['数值']}</font>"}
                )

            columns.append({"tag": "column", "width": "weighted", "weight": 1, "elements": elements})

        return {"tag": "column_set", "flex_mode": "stretch", "columns": columns}

    def _build_curve_section(self, chart_data: list[dict[str, Any]]) -> dict[str, Any]:
        """构建累计收益曲线部分"""
        return {"tag": "chart", "aspect_ratio": "16:9", "chart_spec": self._create_chart_spec(chart_data)}
    ...
    def _create_chart_spec(chart_data: list[dict[str, Any]]) -> dict[str, Any]:
        """创建VChart规格的累计收益曲线图表"""
        mark_point_data = [
            {"type": "max", "name": "最大值", "label": {"visible": True, "style": {"fill": "red"}}},
            {"type": "min", "name": "最小值", "label": {"visible": True, "style": {"fill": "green"}}},
        ]
        ...
        for _s_type, item in last_points.items():
        ...
        return {
            ...
            "markPoint": {"data": mark_point_data, "itemContent": {"offsetY": -10}},
        }


def push_strategy_latest(
    strategy: str,
    dfw: pd.DataFrame,
    feishu_key: str,
    out_sample_sdt="20250101",
    show_recent_detail: bool = True,
    **kwargs,
):
    """推送策略最新表现到飞书群
