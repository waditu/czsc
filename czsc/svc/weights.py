...
def show_weight_ts(df, key=None, **kwargs):
    ...
    fig.update_layout(
        ...
        font={"color": None},  # 使用 Streamlit 主题
    )

    if key is None:
        key = generate_component_key(
            df, prefix="w_ts", title=title, show_position_count=show_position_count, height=height
        )
...
def show_weight_cdf(df, key=None, **kwargs):
    ...
    if key is None:
        key = generate_component_key(
            df, prefix="w_cdf", title=title, show_percentiles=show_percentiles, height=height, width=width
        )
...
def show_weight_abs(df, key=None, **kwargs):
    ...
    fig = plot_absolute_position_analysis(
        df, title=title, height=height, width=width, ma_windows=ma_windows, volatility_window=volatility_window
    )
