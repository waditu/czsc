...
def apply_stats_style(stats_df):
    ...
    style_config = {
        ...
        "positive_indicators": {
            "columns": ["绝对收益", "年化", "夏普", "卡玛", "日胜率", "日盈亏比", "日赢面", "非零覆盖", "新高占比", "单笔收益"],
            "cmap": "RdYlGn_r"
        },
        # 负向指标（越小越好）- 使用 RdYlGn 配色  
        "negative_indicators": {
            "columns": ["最大回撤", "年化波动率", "下行波动率", "盈亏平衡点", "新高间隔", "回撤风险"],
            "cmap": "RdYlGn"
        }
    }
    ...
    for col in style_config["positive_indicators"]["columns"]:
        if col in stats_df.columns:
            stats_styled = stats_styled.background_gradient(
                cmap=style_config["positive_indicators"]["cmap"], 
                axis=None, 
                subset=[col]
            )
    ...
    for col in style_config["negative_indicators"]["columns"]:
        if col in stats_df.columns:
            stats_styled = stats_styled.background_gradient(
                cmap=style_config["negative_indicators"]["cmap"], 
                axis=None, 
                subset=[col]
            )
