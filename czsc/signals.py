...
def get_fx_signals(ka):
    ...
    freq = ka.name
    s = {
        f"{freq}收于MA5上方": False,
        f"{freq}收于MA5下方": False,
        f"{freq}收于MA20上方": False,
        f"{freq}收于MA20下方": False,
        f"{freq}收于MA120上方": False,
        f"{freq}收于MA120下方": False,
        f"{freq}最后一个分型为顶": False,
        f"{freq}最后一个分型为底": False,
        f"{freq}顶分型后有效跌破MA5": False,
        f"{freq}底分型后有效升破MA5": False,
    }
    ...
    if last_k['close'] >= last_k['ma5']:
        s[f"{freq}收于MA5上方"] = True
    else:
        s[f"{freq}收于MA5下方"] = True

    if last_k['close'] >= last_k['ma20']:
        s[f"{freq}收于MA20上方"] = True
    else:
        s[f"{freq}收于MA20下方"] = True

    if last_k['close'] >= last_k['ma120']:
        s[f"{freq}收于MA120上方"] = True
    else:
        s[f"{freq}收于MA120下方"] = True

    last_fx = ka.fx_list[-1]
    after_klines = [x for x in last_klines_ if x['dt'] >= last_fx['dt']]
    if last_fx['fx_mark'] == 'g':
        s[f"{freq}最后一个分型为顶"] = True
        ...
        if after_klines[1]['close'] < after_klines[1]['ma5'] \
            ...
            s[f'{freq}顶分型后有效跌破MA5'] = True

    if last_fx['fx_mark'] == 'd':
        s[f"{freq}最后一个分型为底"] = True
...
def get_bi_signals(ka):
    ...
    freq = ka.name
    s = {
        f"{freq}最后一个未确认的笔标记为底": False,
        f"{freq}最后一个未确认的笔标记为顶": False,
        f"{freq}最后一个已确认的笔标记为底": False,
        f"{freq}最后一个已确认的笔标记为顶": False,
        f"{freq}向上笔走势延伸": False,
        f"{freq}向上笔现顶分型": False,
        f"{freq}向下笔走势延伸": False,
        f"{freq}向下笔现底分型": False,

        f"{freq}最后一个笔中枢上沿": 0,
        f"{freq}最后一个笔中枢下沿": 0,
        f"{freq}收于笔中枢上方且有三买": False,
        f"{freq}收于笔中枢上方且无三买": False,
        f"{freq}收于笔中枢下方且有三卖": False,
        f"{freq}收于笔中枢下方且无三卖": False,

        f'{freq}笔同级别分解买': False,
        f'{freq}笔同级别分解卖': False,

        f'{freq}类趋势顶背驰（笔）': False,
        f'{freq}类趋势底背驰（笔）': False,
        f'{freq}类盘整顶背驰（笔）': False,
        f'{freq}类盘整底背驰（笔）': False,

        # f'{freq}趋势顶背驰（笔）': False,
        # f'{freq}趋势底背驰（笔）': False,
        # f'{freq}盘整顶背驰（笔）': False,
        # f'{freq}盘整底背驰（笔）': False,
    }

    # ------------------------------------------------------------------------------------------------------------------
    assert ka.bi_list[-1]['fx_mark'] in ['d', 'g']
    if ka.bi_list[-1]['fx_mark'] == 'd':
        s[f"{freq}最后一个未确认的笔标记为底"] = True
    else:
        s[f"{freq}最后一个未确认的笔标记为顶"] = True

    assert ka.bi_list[-2]['fx_mark'] in ['d', 'g']
    if ka.bi_list[-2]['fx_mark'] == 'd':
        s[f"{freq}最后一个已确认的笔标记为底"] = True
    else:
        s[f"{freq}最后一个已确认的笔标记为顶"] = True

    # ------------------------------------------------------------------------------------------------------------------
    last_bi = ka.bi_list[-1]
    if last_bi['fx_mark'] == 'd' and ka.fx_list[-1]['fx_mark'] == 'd':
        s[f"{freq}向上笔走势延伸"] = True
    elif last_bi['fx_mark'] == 'd' and ka.fx_list[-1]['fx_mark'] == 'g':
        s[f"{freq}向上笔现顶分型"] = True
    elif last_bi['fx_mark'] == 'g' and ka.fx_list[-1]['fx_mark'] == 'g':
        s[f'{freq}向下笔走势延伸'] = True
    elif last_bi['fx_mark'] == 'g' and ka.fx_list[-1]['fx_mark'] == 'd':
        s[f'{freq}向下笔现底分型'] = True
    else:
    ...
    if bis[-1]['fx_mark'] == 'd' and bis[-1]['bi'] < bis[-3]['bi'] and bis[-2]['bi'] < bis[-4]['bi']:
        ...
        if ka.is_bei_chi(zs1, zs2, mode="bi", adjust=0.9):
            ...
            if bis[-2]['bi'] < bis[-5]['bi']:
                s[f'{freq}类趋势底背驰（笔）'] = True
            else:
                s[f'{freq}类盘整底背驰（笔）'] = True
    ...
    if bis[-1]['fx_mark'] == 'g' and bis[-1]['bi'] > bis[-3]['bi'] and bis[-2]['bi'] > bis[-4]['bi']:
        ...
        if ka.is_bei_chi(zs1, zs2, mode="bi", adjust=0.9):
            # 类趋势
            if bis[-2]['bi'] > bis[-5]['bi']:
                s[f'{freq}类趋势顶背驰（笔）'] = True
            else:
                s[f'{freq}类盘整顶背驰（笔）'] = True

    # ------------------------------------------------------------------------------------------------------------------
    bi_zs = find_zs(bis)
    if bi_zs:
        last_bi_zs = bi_zs[-1]
        s[f"{freq}最后一个笔中枢上沿"] = last_bi_zs['ZG']
        s[f"{freq}最后一个笔中枢下沿"] = last_bi_zs['ZD']

        last_k = ka.kline_new[-1]
        if last_k['close'] > last_bi_zs['ZG']:
            if last_bi_zs.get("third_buy", 0):
                s[f"{freq}收于笔中枢上方且有三买"] = True
            else:
                s[f"{freq}收于笔中枢上方且无三买"] = True

        if last_k['close'] < last_bi_zs['ZD']:
            if last_bi_zs.get("third_sell", 0):
                s[f"{freq}收于笔中枢下方且有三卖"] = True
            else:
                s[f"{freq}收于笔中枢下方且无三卖"] = True

    # ------------------------------------------------------------------------------------------------------------------
    if len(bis) >= 6:
        if bis[-1]['fx_mark'] == 'd' and bis[-2]['bi'] > bis[-5]['bi']:
            if bis[-1]['bi'] > bis[-3]['bi'] or s[f'{freq}类盘整底背驰（笔）']:
                s[f'{freq}笔同级别分解买'] = True

        if bis[-1]['fx_mark'] == 'g' and bis[-2]['bi'] < bis[-5]['bi']:
            if bis[-1]['bi'] < bis[-3]['bi'] or s[f'{freq}类盘整顶背驰（笔）']:
                s[f'{freq}笔同级别分解卖'] = True
    return s
...
def get_xd_signals(ka, use_zs=False):
    ...
    freq = ka.name
    s = {
        f"{freq}最后一个未确认的线段标记为底": False,
        f"{freq}最后一个未确认的线段标记为顶": False,
        f"{freq}最后一个已确认的线段标记为底": False,
        f"{freq}最后一个已确认的线段标记为顶": False,

        f"{freq}最后一个线段内部笔标记数量": 0,
        f"{freq}最近上一线段内部笔标记数量": 0,

        f'{freq}类趋势顶背驰（段）': False,
        f'{freq}类趋势底背驰（段）': False,
        f'{freq}类盘整顶背驰（段）': False,
        f'{freq}类盘整底背驰（段）': False,

        f"{freq}同级别分解买": False,
        f"{freq}同级别分解卖": False,

        # f'{freq}趋势顶背驰（段）': False,
        # f'{freq}趋势底背驰（段）': False,
        # f'{freq}盘整顶背驰（段）': False,
        # f'{freq}盘整底背驰（段）': False,
        # f"{freq}最后一个中枢上沿": 0,
        # f"{freq}最后一个中枢下沿": 0,
    }
    ...
    if ka.xd_list[-1]['fx_mark'] == 'd':
        s[f"{freq}最后一个未确认的线段标记为底"] = True
    else:
        s[f"{freq}最后一个未确认的线段标记为顶"] = True
    ...
    s[f"{freq}最后一个线段内部笔标记数量"] = len(bi_after)
    s[f"{freq}最近上一线段内部笔标记数量"] = len([x for x in ka.bi_list[-100:]
    ...
    if xds[-1]['fx_mark'] == 'd' and xds[-1]['xd'] < xds[-3]['xd'] and xds[-2]['xd'] < xds[-4]['xd']:
        ...
        if ka.is_bei_chi(zs1, zs2, mode="xd", adjust=0.9):
            ...
            if xds[-2]['xd'] < xds[-5]['xd']:
                s[f'{freq}类趋势底背驰（段）'] = True
            else:
                s[f'{freq}类盘整底背驰（段）'] = True
    ...
    if xds[-1]['fx_mark'] == 'g' and xds[-1]['xd'] > xds[-3]['xd'] and xds[-2]['xd'] > xds[-4]['xd']:
        ...
        if ka.is_bei_chi(zs1, zs2, mode="xd", adjust=0.9):
            # 类趋势
            if xds[-2]['xd'] > xds[-5]['xd']:
                s[f'{freq}类趋势顶背驰（段）'] = True
            else:
                s[f'{freq}类盘整顶背驰（段）'] = True
    ...
    if len(xds) >= 6 and len(last_xd_inside) >= 6:
        if xds[-1]['fx_mark'] == 'g' and xds[-2]['xd'] < xds[-5]['xd']:
            if xds[-1]['xd'] < xds[-3]['xd'] or s[f'{freq}类盘整底背驰（段）']:
                s[f'{freq}同级别分解买'] = True

        if xds[-1]['fx_mark'] == 'd' and xds[-2]['xd'] > xds[-5]['xd']:
            if xds[-1]['xd'] > xds[-3]['xd'] or s[f'{freq}类盘整顶背驰（段）']:
                s[f'{freq}同级别分解卖'] = True
...
class Signals:
    ...
    def __repr__(self):
        return f"<Signals of {self.symbol}>"
